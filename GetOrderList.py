...
import datetime
import pymysql
from config import phonelist as P
...
class Phone(object):
    ...
    def __init__(self):
        ...
        self.orderlast[0]['billAmount'], self.orderlast[0]['billName'], self.orderlast[0]['timeInfo'] = self.sql_search(account)
    ...
    def findbill(self):
        ...
        for bill in bills:
            ...
            # input(billdata)
            if billdata in self.ordernew:  # 如果已在本次订单列表中，即滑屏导致的重复读取，则跳过该笔订单
                continue
            elif billdata in self.orderlast:  # 如果已在上次发送的订单列表中，则结束读取
                return 0  # 返回0，告知读取完毕
            else:
                # input('否则继续查找')
                self.ordernew.append(billdata)  # 否则添加到新订单列表，继续查找
        info('all bills are new, continue find')
        self.pages += 1
        self.ct.swipe(0.5, 0.85, 0.5, 0.2, 0.5)  # 上滑查看更多订单
        return 1  # 返回1，继续获取

    def get(self):
        info('start get new order')
        flag = self.findbill()  # 先读取一次
        while flag:  # 如果全屏都是新订单，就重复执行
            flag = self.findbill()
        info('Get complete')
        print('New order: ', self.ordernew)
        if self.ordernew:
            with open('record\\record_%s.txt' % account, 'a') as f:  # 储存在本地
    ...
    def sql_conn(cls):  # 连接数据库
        cls.mysql = pymysql.connect(host=db['host'], port=db['port'], user=db['user'], passwd=db['passwd'], db=db['dbname'])
...
if __name__ == '__main__':
    print('\n----------地址配置如下----------')
    for i, p in zip(range(10), P):
        print(i, '--', p)
    while 1:
        ip, account = P[0]['ip'], P[0]['user']+P[0]['account']  # 默认使用配置0
        Input = input('请输入编号选择配置: ')
        if Input == '':
            print('未选择，使用默认配置0')
        elif Input in '0123456789':
            ip, account = P[int(Input)]['ip'], P[int(Input)]['user']+P[int(Input)]['account']
        else:
            print('输入有误,请重新选择')
            continue
        GG = input('ip: %s  account: %s  输入GG继续,否则请重新选择\n' % (ip, account))
        if GG == 'GG':
            break
        continue
    ...
    logging.basicConfig(level=logging.INFO,
                        ...
                        datefmt='%Y-%m-%d %H:%M:%S',
                        filename='log\log_%s.log' % account,
                        filemode='a')
